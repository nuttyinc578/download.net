...
[Website](https://nuttyinc578.github.io/download.net/) · [Download installer v1.0.4](https://github.com/nuttyinc578/download.net/releases/download/v1.0.4/download.net-Setup-1.0.4.exe) · [Portable folder](https://github.com/nuttyinc578/download.net/releases/download/v1.0.4/download.net-launcher-v1-folder.zip) · [Nightly download](https://nightly.link/nuttyinc578/download.net/workflows/nightly.yml/main/download.net-launcher-v1.zip) · [MIT license](LICENSE) · [Code of conduct](CODE_OF_CONDUCT.md) · [Contributing](CONTRIBUTING.md)

The v1.0.4 installer and portable folder include the current launcher and a Nuttyinc backend that starts automatically on this PC. No manual connection setup or separate developer SDKs are needed. Nightly builds are also produced by GitHub Actions; if nightly.link is unavailable, use the permanent release downloads. The catalog includes verified, maintainer-approved submissions, and accounts are local to this PC.

## Installation folder recovery

Version 1.0.4 checks the app destination before downloading. Protected folders such as Program Files automatically fall back to your Windows account's `%LOCALAPPDATA%\Programs` folder. Settings → **Use recommended folder** restores this location at any time. The launcher saves folder changes only after checking access. Existing installed app folders remain untouched.
