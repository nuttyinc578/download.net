...
test('backend account lifecycle and signed one-use bootstrap verification',async t=>{
 ...
 assert.equal((await fetch(origin+'/api/apps')).status,503);
 await writeFile(catalogFile,JSON.stringify([listing]));
 const appsResponse=await fetch(origin+'/api/apps');assert.equal(appsResponse.status,200);assert.equal((await appsResponse.json()).apps[0].url,listing.url);
