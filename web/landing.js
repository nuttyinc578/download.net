...
button.addEventListener('click',()=>{if(ready&&agreement.checked)location.href='https://nightly.link/nuttyinc578/download.net/workflows/nightly.yml/main/download.net-launcher-v1.zip';});
(async()=>{
 const status=document.querySelector('#release-status');
 try{
  const response=await fetch('https://api.github.com/repos/nuttyinc578/download.net/actions/workflows/nightly.yml/runs?branch=main&status=success&per_page=1',{signal:AbortSignal.timeout(12000)});
  if(!response.ok)throw Error();
  const data=await response.json();
  if(!data.workflow_runs?.length)throw Error();
  const artifacts=await fetch(`https://api.github.com/repos/nuttyinc578/download.net/actions/runs/${data.workflow_runs[0].id}/artifacts`,{signal:AbortSignal.timeout(12000)});
